...
class _EntryPageState extends State<EntryPage> {
  ...
  Future<void> _loadJournalDetails() async {
    ...
    try {
        ...
        if (!currentPaletteElementExists && _currentJournalDetails!.palette.colors.isNotEmpty) {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text("La couleur d'origine de cette note n'existe plus dans la palette. Une couleur par défaut a été sélectionnée."), backgroundColor: Colors.orange),
            );
          }
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text("La palette de ce journal est vide. Veuillez ajouter des couleurs à la palette pour pouvoir associer cette note."), backgroundColor: Colors.red),
            );
          }
        }
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Erreur chargement détails journal: ${e.toString()}")));
      }
    } finally {
      if (mounted) {
        setState(() {
  ...
  Future<void> _selectEventDate(BuildContext context) async {
    final DateTime? picked = await showDatePicker(
      ...
      // Allow dates up to 50 years in the past
      lastDate: DateTime(DateTime.now().year + 50),
      // Allow dates up to 50 years in the future
  ...
  void _setDateTimeToNow() {
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Date et heure réglées sur maintenant."), duration: Duration(seconds: 2)));
    }
  }
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          if (_isSaving || _isSavingAsNew)
            ...
            // Save button
            IconButton(icon: const Icon(Icons.save_alt_outlined), onPressed: _saveNote, tooltip: isEditing ? "Mettre à jour la note" : "Sauvegarder la note"),
        ],
      ),
      body:
          _isLoadingJournalDetails
              ? const Center(child: CircularProgressIndicator())
              : _currentJournalDetails == null
              ? Center(
                // Display error if journal details could not be loaded
                child: Column(
                    ...
                    children: <Widget>[
                      ...
                      if (_currentJournalDetails!.palette.colors.isNotEmpty)
                        DropdownButtonFormField<String>(
                          ...
                          decoration: InputDecoration(labelText: 'Couleur associée', border: OutlineInputBorder(borderRadius: BorderRadius.circular(8))),
                          items:
                              _currentJournalDetails!.palette.colors.map((ColorData colorData) {
