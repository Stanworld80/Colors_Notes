...
_preparedColors = [ // Provide a very basic default if no sources are available
...
if (mounted) {
ScaffoldMessenger.of(context).showSnackBar(
SnackBar(content: Text('Un journal nommé "$journalName" existe déjà.'), backgroundColor: Colors.orange),
);
}
...
if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("La palette en préparation est vide. Veuillez ajouter des couleurs ou choisir une source avec des couleurs.")));
return;
}
...
if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("La palette doit contenir entre ${MIN_COLORS_IN_PALETTE_EDITOR} et ${MAX_COLORS_IN_PALETTE_EDITOR} couleurs.")));
return;
}
...
labelText: 'Nom du nouveau journal',
...
_preparedPaletteName = trimmedValue.isNotEmpty ? "Palette de '$trimmedValue' (Modèle: ${_selectedPaletteModel!.name})" : "Palette (Modèle: ${_selectedPaletteModel!.name})";
} else if (_creationMode == JournalCreationMode.fromExistingJournal && _selectedExistingJournal != null) {
_preparedPaletteName = trimmedValue.isNotEmpty ? "Palette de '$trimmedValue' (Copiée de ${_selectedExistingJournal!.name})" : "Palette (Copiée de ${_selectedExistingJournal!.name})";
} else {
